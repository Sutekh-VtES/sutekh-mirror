# CardListProfileEditor.py
# -*- coding: utf8 -*-
# vim:fileencoding=utf8 ai ts=4 sts=4 et sw=4
# Dialog for editing per-frame/per-deck profiles.
# Copyright 2010 Simon Cross <hodgestar+sutekh@gmail.com>
# Copyright 2010 Neil Muller <drnlmullr+sutekh@gmail.com>
# GPL - see COPYING for details

"""Edit the profile for the WW Cardlist"""
...
class CardListProfileEditor(SutekhDialog):
    """Dialog which allows the user to edit WW cardlist profiles.
       """
    ...
    def __init__(self, oParent, oConfig):
        super(CardListProfileEditor, self).__init__("Edit Cardlist Profiles",
        ...
        for sKey in self.__oConfig.cardlist_options():
            ...
            aOptions.append((sKey,
                self.__oConfig.get_cardlist_option_spec(sKey), bInherit))
    ...
    def _button_response(self, _oWidget, iResponse):
        ...
        if self._check_unsaved_changes():
            ...
            # Evil recursion
            self.destroy()
            oDlg = CardListProfileEditor(self.__oParent, self.__oConfig)
            oDlg.run()

    def _all_profile_keys(self):
        """Return a set of all profile keys (including unsaved profiles)."""
        aProfiles = set(self.__oConfig.profiles(WW_CARDLIST))
    ...
    def _profile_options(self, sProfile):
        ...
        elif sProfile in self.__oConfig.profiles(WW_CARDLIST) or \
                sProfile == "defaults":
            dValues = {}
    ...
    def _save_unsaved_changes(self):
        ...
        for sProfile, dValues in self.__dUnsavedChanges.items():
            ...
            for sKey, sValue in dValues.items():
                self.__oConfig.set_cardlist_profile_option(sProfile,
                        sKey, sValue)
