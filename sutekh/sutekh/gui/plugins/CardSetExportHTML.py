...
class CardSetExportHTML(CardListPlugin):
    ...
    def gen_tree(self, oCardSet, bDoText):
        ...
        oBody = _add_header(oDocRoot, oCardSet)

        dCards = self.get_cards()
        aSortedVampires = _add_crypt(oBody, dCards)
        aSortedLibCards = _add_library(oBody, dCards)
        if bDoText:
            ...
            _add_crypt_text(oCardText, aSortedVampires)
            _add_library_text(oCardText, aSortedLibCards)
...
# functions to fill in the actual HTML content
def _add_header(oDocRoot, oCardSet):
...
def _add_crypt(oBody, dCards):
    ...
    def add_row(oCryptTBody, tVampInfo, oCard):
        ...
        # Disciplines
        oTD = SubElement(oTR, "td")
        _add_span(oTD, get_disciplines(oCard), 'tablevalue')
        # Title
        oTD = SubElement(oTR, "td")
...
def _add_crypt_text(oCardText, aSortedVampires):
    ...
    for tVampInfo, oCard in aSortedVampires:
        ...
        _add_span(oListItem, get_disciplines(oCard), 'disciplines')
        # Text
        _add_text(oCardText, oCard)

def _add_library_text(oCardText, aSortedLibCards):
